...
namespace CSharpFilters
{
	...
	public class ColorSpace
	{
		...
		public static Bitmap Hue(Bitmap source, float hueFactor, float saturationFactor = 1F, float luminanceFactor = 1F)
		{
			int width = source.Width;
			int height = source.Height;
			...
			if (source.PixelFormat != PixelFormat.Format32bppArgb) source = source.Clone(rc, PixelFormat.Format32bppArgb);

			Bitmap dest = new Bitmap(width, height, source.PixelFormat);

			BitmapData dataSrc = source.LockBits(rc, ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
			BitmapData dataDest = dest.LockBits(rc, ImageLockMode.ReadWrite, PixelFormat.Format32bppArgb);

			int offset = dataSrc.Stride - (width*4);
			...
			unsafe
			{
				...
				for (int y = 0; y < height; ++y)
				{
					for (int x = 0; x < width; ++x)
					{
						HSL hsl = HSL.FromRGB(bytesSrc[2], bytesSrc[1], bytesSrc[0]); // Still BGR
						hsl.Hue *= hueFactor;
                        hsl.Luminance *= luminanceFactor;
                        hsl.Saturation *= saturationFactor;
