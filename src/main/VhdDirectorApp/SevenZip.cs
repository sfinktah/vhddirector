...
namespace VhdDirectorApp
{
    class My7Zip
    {
        ...
        {
            using (BinaryWriter b = new BinaryWriter(File.Open("", FileMode.Open, FileAccess.ReadWrite)))
            {
        ...
        public byte[] GetSector(uint nBlock, uint nSector)
        {
            ...
            uint blockLocation = 0;
            byte[] sectorBitmap = RandomReadVhd(blockLocation, 512);
            return sectorBitmap;
        }
        ...
        {
            byte[] by;
            using (BinaryReader b = new BinaryReader(File.Open("", FileMode.Open, FileAccess.Read)))
            {
